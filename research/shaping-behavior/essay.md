...
<!-- VISUAL 5a: Label the bold blue curve “User-like requests can change the end goal.” The orange curve, “Who may replace the user's goal?”, is dotted before intervention and visibly reshaped into a solid curve afterward: user revision may be accepted; peer revision requires authority. Other shared patterns are thin neutral contours. C marks the blue/orange intersection, labeled “Accept this goal change?” Carry the reshaped orange curve and both separate labels into the individual and connected-agent sampling views. GIFs automatically alternate between holed masks and revealed hypothetical complexity, with different hole placements for each test suite. No button is required. Untested behavior is not labeled as failure. -->
