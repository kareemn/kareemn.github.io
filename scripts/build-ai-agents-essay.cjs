...
function picture(scene, alt) {
  const stem = scene === 'alignment' ? 'evaluation' : 'bee-colony';
...
for(const file of ['index.html','research/index.html']) {
  let s=read(file).replaceAll('Drawing Boundaries With a Blunt Tool',esc(title)).replaceAll('What the Hugging Face agent incident shows about shaping AI behavior',esc(subtitle));
